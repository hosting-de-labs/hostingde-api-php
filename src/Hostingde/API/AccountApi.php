...
use Hostingde\API\classes\account\Account;
use Hostingde\API\classes\account\AccountUser;
use Hostingde\API\classes\account\RightsTemplate;
...
class AccountApi extends GenericApi {
	...
	public function subaccountUpdate($subaccount) {
		$data = array('authToken' => $this->authToken, 'subaccount' => $subaccount);

		$this->send('subaccountUpdate', $data);
		if ($this->getStatus() == "error") {
			return false;
		}
		return new Account($this->getValue());
